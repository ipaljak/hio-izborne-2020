...
int main() {
    ...
    for (int i = 0; i <= n; i++) {
        for (int j = 0; j < m; j++)
            if (i % 3 == 0 || i >= n - (n % 3)) add_edge(i, j, 'R');
            else if ((i % 3) + (2 * j / m) == 2) add_edge(i, j, 'R');
            else if (j < m / 3) add_edge(i, j, 'U');
            else if (j < m / 2) add_edge(i, j, 'D');
            else if (j <= 2 * m / 3) add_edge(i, j, 'U');
            else add_edge(i, j, 'D');
        if (i != n) add_edge(i, m / 2, 'D');
        if (i != 0 && i % 3 == 0) add_edge(i, m, 'U');
    }
