...
Solution dva_n_m() {
    ...
    for (int i = 0; i <= n; i++) {
        for (int j = 0; j < m; j++)
            if (i % 3 == 0 || i >= n - (n % 3)) S.add_edge(i, j, 'R');
            else if ((i % 3) + (2 * j / m) == 2) S.add_edge(i, j, 'R');
            else if (j < m / 3) S.add_edge(i, j, 'U');
            else if (j < m / 2) S.add_edge(i, j, 'D');
            else if (j <= 2 * m / 3) S.add_edge(i, j, 'U');
            else S.add_edge(i, j, 'D');
        if (i != n) S.add_edge(i, m / 2, 'D');
        if (i != 0 && i % 3 == 0) S.add_edge(i, m, 'U');
    }
    return S;
}
...
int main() {
    ...
    int score = min({S1.score(), S2.score(), S3.score()});
    if (S1.score() == score) S1.print();
    else if (S2.score() == score) S2.print();
    else S3.print();
